                ...
                <div class="panel br-6 p-0">
                    <div class="custom-table">
                        <div class="table-header">
                            ...
                            <b-modal :hide-footer="true" id="exampleModalCenter" :title="in_edit ? 'Editar empresa' : 'Cadastrar empresa'" centered>
                                
                                <b-form class="simple-example" novalidate @submit.stop.prevent="record">
                                        <b-form-row>
                                            <div class="col-md-12 mb-4">
                                                <label for="fullName">Razão Social</label>
                                                <b-input id="fullName" v-model="form.name" placeholder="" :class="[submit_form ? (form.name ? 'is-valid' : 'is-invalid') : '']"></b-input>
                                                
                                                <b-form-invalid-feedback :class="{'d-block' : submit_form && !form.name}">Informe a razão social</b-form-invalid-feedback>
                                            </div>

                                            <div class="col-md-6 mb-4">
                                                <label for="fullName">CNPJ</label>
                                                <b-input id="fullName" v-mask="'###.###.###/####-##'" placeholder="___.___.___/____-__" v-model="form.cnpj" :class="[submit_form ? (form.cnpj ? 'is-valid' : 'is-invalid') : '']"></b-input>
                                                
                                                <b-form-invalid-feedback :class="{'d-block' : submit_form && !form.warranty_date}">Informe o cnpj</b-form-invalid-feedback>
                                            </div>
                                        </b-form-row>
                        ...
                        <b-table emptyText="Nenhuma empresa encontrada." ref="basic_table" responsive hover :items="items" :fields="columns" :per-page="table_option.page_size" :current-page="table_option.current_page" :filter="table_option.search_text"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            record(){
                ...
                if(this.validation()){
                    ...
                    if(this.in_edit){
                        ...
                        axios.put('/api/enterprise/'+this.form.id, this.form,{
                                headers: {
                        ...
                        axios.post('/api/enterprise', this.form,{
                            headers: {
                                'Authorization': `Bearer ${this.token}`
                            }
                        })
            ...
            delete_row(item) {
                if (confirm('Deseja excluir a empresa '+item.name+'?')) {

                    axios.delete('/api/enterprise/'+item.id)
                    .then(response => {
                    ...
                    .catch(error => {
                        ...
                        this.$swal({
                            title: 'Erro ao excluir empresa. Excluir lotes cadastrados para essa empresa.',
                            padding: '2em'
                        });
                    });
                }
